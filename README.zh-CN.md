...
`FortuneContract` 提供 `drawFortune`、`canDraw`、`getLatestFortune` 和抽签记录读取接口，发出 `FortuneDrawn` 事件，并限制每个钱包按 UTC 自然日最多抽取五次。
...
3. 开启 Email Magic Link 登录。
4. 如需 Google 登录，配置 OAuth 并设置 `VITE_ENABLE_GOOGLE_AUTH=true`。
5. 部署 `supabase/functions/wallet-claim`。
6. 为函数添加 `SUPABASE_URL`、`SUPABASE_SERVICE_ROLE_KEY`、`MONAD_RPC_URL` 和 `CONTRACT_ADDRESS` secrets。
