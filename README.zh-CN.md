...
- 每个钱包每日最多抽取十签，00:00 刷新；未连接钱包的访客可体验五次。
...
`FortuneContract` 提供 `drawFortune`、`canDraw`、`getLatestFortune` 和抽签记录读取接口，发出 `FortuneDrawn` 事件，并限制每个钱包每日最多抽取十次。

当前 Monad Testnet 部署：

```text
合约地址：0x02f76e26db659657b7543898f428917a2a50caea
部署交易：0xe8e6428b0ebfaa00df95b6469604495c02d492ebdd749deebe68e3eceac90f6c
区块高度：45911686
```

[在 Monad Explorer 查看已部署合约](https://testnet.monadexplorer.com/address/0x02f76e26db659657b7543898f428917a2a50caea)
