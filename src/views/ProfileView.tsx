...
export function ProfileView() {
  const { profile, setProfile, history, cloudSyncing, lastCloudSync, syncCloudHistory, resetToGuestSession } = useOmikuji();
  ...
  async function requestCode() {
    ...
    try {
      ...
      if (result.demo) {
        setProfile({ ...profile, email, signedIn: true, id: "demo-user" });
  ...
  async function confirmCode() {
    ...
    try {
      ...
      setProfile({ ...profile, email, signedIn: true, id: result.user?.id ?? "demo-user" });
      const synced = await syncCloudHistory();
      setCodeSent(false);
      setEmailCode("");
      setNotice(`登录成功，已同步 ${synced} 份账号御签。`);
